...
const YourStack = ({stack,removeAll,removeOne}) => {
    return (
...
<div className='border rounded-lg mt-3 py-6 text-center text-gray-300'>
    Your Stack is empty.
    </div>
</>
):(
...
<p>{item.name}</p>
<RxCrossCircled className='text-gray-400 cursor-pointer' onClick={()=> removeOne(item.name)}/>
</div>
);
})}
